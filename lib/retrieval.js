...
function splitSections(docs, { sourceFile = "docs.md", startIndex = 0 } = {}) {
...
export function retrieveDocs({ docs, sourcesText, topic, maxSections = 4, referenceText = "", reference = "" }) {
  const resolvedReference = referenceText || reference;
  const docSections = splitSections(docs, { sourceFile: "docs.md" });
  const referenceSections = resolvedReference.trim()
    ? splitSections(resolvedReference, { sourceFile: "reference.md", startIndex: docSections.length })
    : [];
  const sections = [...docSections, ...referenceSections];
...
export function buildRetrievalText({ docs, referenceText, reference }) {
  const resolvedReference = referenceText || reference || "";
  return [
    String(docs || "").trim(),
    resolvedReference.trim() ? `# Reference Details\n\n${resolvedReference.trim()}` : ""
  ]
    .filter(Boolean)
    .join("\n\n");
}

export function formatFullDocsResult({ docs, sourcesText, referenceText, reference, includeSources = true }) {
  const parts = [buildRetrievalText({ docs, referenceText, reference })];
...
export function formatDocsResult(result) {
  ...
  if (result.query) {
    ...
    const matchedFiles = [...new Set(result.selected.map((section) => section.sourceFile).filter(Boolean))];
    if (matchedFiles.includes("reference.md")) notes.push(`Matched files: ${matchedFiles.join(", ")}`);
  ...
  return registry.tools
    .map((tool) => {
      ...
      if (includeDocs) {
        try {
          const toolRoot = path.join(root, tool.path);
          docs = buildRetrievalText({
            docs: fs.readFileSync(path.join(toolRoot, "docs.md"), "utf8"),
            referenceText: loadOptionalText(path.join(toolRoot, "reference.md"))
          });
        } catch {
          docs = "";
        }
