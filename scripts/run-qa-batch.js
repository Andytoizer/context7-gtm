...
const command = process.env.QA_SWARM_COMMAND || "";
if (command && !options.dryRun) {
  runExternal(command, {
    MAINTENANCE_INPUT: scoutPath,
    MAINTENANCE_OUTPUT: outputPath,
    MAINTENANCE_PROMPT: promptPath,
    MAINTENANCE_RUN_ID: scout.runId,
    MAINTENANCE_BATCH_ID: scout.batchId,
  });
  assertJsonFile(outputPath);
} else {
  writeJson(outputPath, buildPendingQaOutput(scout));
}

console.log(`Wrote QA verdicts for ${scout.batchId} to ${path.relative(root, outputPath)}`);
...
function buildPendingQaOutput(scout) {
  return {
    runId: scout.runId,
    batchId: scout.batchId,
    checkedAt: new Date().toISOString(),
    status: "pending-run",
    note: "No QA_SWARM_COMMAND was configured, so this batch wrote placeholder QA verdicts.",
    verdicts: (scout.tools || []).map((tool) => ({
      toolId: tool.toolId,
      slug: tool.slug,
      verdict: "pending-run",
      approvedFindings: [],
      rejectedFindings: [],
      qaNotes: "QA swarm not configured for this run.",
      evidenceIssues: [],
    })),
  };
}
